...
- ## 3장 컨트롤러는 진입점의 역할만 수행해야한다 
	...
	- #### 3. new대신 스프링컨테이너가 왜 필요한지 좋은 코드와 어떻게 연관이 있는지 이해한다.
		...
		- **1️⃣ 처음에는 BookRepository를 직접 구현체로 사용**
			
			- 책 정보를 저장하는 BookMemoryRepository를 만들고, 코드에서 **new BookMemoryRepository()** 로 직접 생성해서 사용했어요.
			...
			• 나중에 진짜 DB를 사용하는 BookSqlRepository로 바꾸고 싶어도
			
			new BookMemoryRepository()가 **코드 500군데**에 박혀 있으면
			
			👉 전부 찾아서 new BookSqlRepository()로 수정해야 해요 😱
			
			
			- **2️⃣ 그래서 인터페이스 도입!**
				• BookRepository라는 인터페이스를 만들고,
				
				BookMemoryRepository, BookSqlRepository가 이걸 **implements** 하게 했어요.
				
				• 그리고 코드에서는 구현체가 아닌 인터페이스인 BookRepository를 쓰도록 변경!
				
				  
				
				👍 이렇게 하면 **사용하는 쪽의 코드는 변경 안 해도 되고**,
				
				새로운 구현체를 넣기만 하면 되니까 더 유연하죠.
				
			**3️⃣ 그런데 여전히 new BookSqlRepository() 를 써야 했음**
			
				• 인터페이스 도입으로 사용 코드는 바꾸지 않아도 되지만,
				
				여전히 **new BookSqlRepository()** 로 객체를 생성해야 했어요.
				
				• 👉 즉, new를 쓰는 한 **구현체를 바꾸려면 여전히 모든 생성 코드 수정** 필요!
				
			
			- **4️⃣ 그래서 등장한 스프링 컨테이너 (DI)**
				
				• **스프링이 객체를 대신 생성하고 관리**해주는 컨테이너를 활용
				
				• @Repository, @Service, @Autowired 등의 어노테이션을 붙이면,
				
				스프링이 자동으로 필요한 객체를 생성해서 넣어줘요
				
			✅ 덕분에 우리는 그냥 **인터페이스 타입만 선언**하고,
			
			어떤 구현체를 쓸지는 스프링이 주입해주기만 하면 끝!
			...
			**✨ 최종 요약**
			...
			💡 처음엔 new로 생성 → 인터페이스 도입 → 그래도 new는 필요
			
			👉 스프링 컨테이너를 활용하면 **new 없이도 구현체 교체 가능**
