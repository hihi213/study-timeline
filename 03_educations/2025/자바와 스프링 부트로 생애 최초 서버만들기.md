...
- ### 단축키 정리
	...
	- 키암기팁
		...
		- fn: 방향/기능 보조
	- 인텔리제이
		- 실제코드 들어가기:  command클릭/command+B
		- option+f1(왼쪽에 저격수 아이콘): 현재 선택한 클래스나 파일이 **왼쪽 프로젝트 구조 창에서 어디 있는지** 보여줍니다.
		- 파일 검색 단축키: command+shift+o 
		- 현재줄을 위아래로 이동: option+shift+ 방향키
		- 새로운 패키지/파일..등 만들기: cmd+n 
		-  범위지정 :option+⬆️
			- 계속화살표위 누르면 범위가 커진다.
		- import문 생성: option+enter
		- 안쓰는 import문 삭제 control+option+o 
		- 최근에 쓴 클래스 목록: command+E
		- 변수이름 한번에 바꾸기: shift+ f6
	- myspl
		- 코드정리 단축기: option+command+I
	- 캡쳐
		...
		1.  HTTP  **요청** 
			...
			- 필수: 첫번째 줄(요청라인) 
				- HTTPMethod: 야 넌 이 동작해줘
					...
					- POST(이 데이터(보안)받고 데이터 생성하거나, 행동해 , 바디): 데이터를 서버에 전송
...
- ## 2_1. 디스크와 메모리의 차이를 이해하고 Database의 필요성을 이해한다.
...
- ## 2_2. MySQL Database를 SQL과 함께 조작할 수 있다.
	- 먼저 MySQL를 가동해야한다
		- 인텔리제이 유료 사용시
			- 오른쪽 위 Database클릭
			- 실행 Command+ enter
...
- ## 2_3. 스프링 서버를 이용해 Database에 접근하고 데이터를 저장, 조회, 업데이트, 삭제할 수 있다.
	- 2번은 사람이 직접 접근했던 방법
		...
		- ###### 3_5) SQL를 만들어 문자열변수로 만들고 jdbcTemplate로 조작한다
			- 유저생성API
				...
				- 받을 요청
					...
					- HTTPBODY: 
						...
						- }
				- 우리가 할 응답
					- 작성할 코드 x (HTTP 상태 200 OK면 충분)
					- 아무 값도 반환하지 않으면 HTTP 200 OK 응답 전송됨
				```Java
				#예시 유저 생성api
			...
			- 조회 API
				...
				- 우리가 할 응답
					...
					- 이때 우리가 코드로 유저객체List를 반환하기만 하면 spring이 자동으로 내부적으로 **Jackson**을 이용해 **자동으로 JSON 변환** 해주고, HTTP 응답을 작성해줌
					- 자세히 말하자면 **우리가 리턴한 Java 객체(List\<UserResponse>)를** **@RestController + @ResponseBody 조합**이 감지하고 **HTTP 응답을 생성해줘야겠다고 DispatcherServlet이 판단** -> 내부적으로 **HttpMessageConverter** 를 사용해서 **Jackson 라이브러리**를 통해 Java 객체 → JSON 문자열로 **변환** , 변환된 JSON을 **HTTP 응답 바디에 담음** ->HTTP 응답 전체를 완성해서 클라이언트에게 전송
				...
				- JdbcTemplate.query()는 query()는 SQL을 DB에 전달해서,“이 조건에 맞는 데이터를 주세요!"라는 **질문(Query)** 을 날리는 메서드
					...
					- query()는 클라이언트에게 줄 데이터가 결과가 없을 수도 있고, 여러 건일 수도 있을 때 적합하고(전체 유저 조회,키워드 검색), queryForObject는 정확히 하나의 값만 나오는 쿼리에 사용한다 
					- queryForObjec은 RowMapper를  **하나의 행 전체를 자바 객체로 받고 싶을 때** 사용하고, 하나의 필드값만 가져올때는 사용 안한다.
					- 그니까 자바의 메서드의 반환타입은 한타입만 지정이 가능하고, 그래서 한 행같이 필드를 반환할때는 하나의 객체타입으로 반환해야하는데, 객체는 개발자 설계에 따라 (id,name만, id,name,age만, 모두다)이런식으로 다르게 정의되기 때문에 spring은 자동으로 객체로 만들어줄수없다. 그래서 우리가 변환방식을 정의하는 RowMapper가 필요하다
				- RowMapper는 DB 테이블의 한 줄(Row) 을 Java 객체로 바꾸는 도구이고, maprow()를 오버라이딩하기위한 인터페이스
					...
					- maprow(): 안의 로직은 우리가 설정한 sql의 조건에 부합한 행이 있을때만 실행됨
						- ResultSet의 한줄을 Java 객체(T)로 변환하는 메서드(변환로직은 우리가 작성해야함)
				- <총정리>
				- 1.JdbcTemplate.query()가 SQL을 DB에 전달하고	
				- 2.DB는 조건에 맞는 데이터를 찾아 결과를 보내고 그 결과를 ResultSet 형태로 받아옴
				- 3. query()가 ResultSet을 한행씩 순회하면서 각행을RowMapper의 mapRow() 메서드로 넘겨주는 구조이고, 만약 ResultSet이 비어있으면 호출하지 않는다.
				- 3. mapRow() 메서드는 그 한줄을 UserResponse 객체로 바꿔서 리턴한다
			...
			- 업데이트 API
				...
				- 받을 요청
					...
					- HTTP BODY: 
						...
						- }
				- 우리가 할 응답
					- x (아무 값도 반환하지 않으면 HTTP 200 OK 응답 전송됨)
				```Java
				#예시 업데이트 API
			...
			- 삭제 API
				...
				- 받을 요청
					- HTTP Method: DELETE
					- HTTP Path: /user
					- 쿼리사용: 문자열 name(삭제할 사용자이름)
						- 쿼리받는건 @RequestParam 붙여주면~
					- HTTP BODY: x
				- 우리가 할 응답
					- x (아무 값도 반환하지 않으면 HTTP 200 OK 응답 전송됨)
				```Java
				...
				```
- ## 2_4. API의 예외 상황을 알아보고 예외를 처리할 수 있다.
	- 아니 근데 존재하지 않은 유저를 삭제, 수정한다는 요청을 받아도 함수가 정상적으로 종료했다, 성공했다는 200ok로 응답하네?
	- 만약에 함수도중에 우리가 Exception를 반환한다면 500 Internal Server Error를 응답을 한다.
	-  queryForObject()는 반환할 결과없을때 빈 리스트(List)를 반환
		- 그래서 예외처리할때 반환된 리스트.isEmpty()로 처리
	- 반환할 결과없을때 **queryForObject()** EmptyResultDataAccessException예외 발생
		- 결과가 2개이상일때도 예외 발생
		- 그래서  **queryForObject()** 는 trycatch로 두예외를 처리한다.
	- 아래는 강사의 예시인데 id가진 유저가 있다면 0이담긴 list가 나오고, 없다면 빈 list가 나온다 이렇게 처리한다~근데 지피티 말로는  **queryForObject()** 를 더 실무에서 쓴다는것같다.
		```Java
		#예시 업데이트 API 수정 예시(지피티 말로는 실무에선 이렇게 안하는듯..?)
		
		@PutMapping("/user")  
		public void UpdateUser(@RequestBody UserUpdateRequest request){  
			List<Integer> result = jdbcTemplate.query(readSql, new RowMapper<Integer>() {
				@Override
...
- ## 2장 스스로 실습해서 배운것 정리
	...
	- 기본생성자: 객체만 ‘비어 있는 상태’로 생성→ 나중에 setter로 값을 채움
	- 매걔변수있는 생성자:  **객체 생성 시점에 값을 한 번에 주입**
		- 불변객체를 만들때 이때 final필드와 매걔변수 있는 생성자를 만들어준다
		- 개발자가 new를 사용할때(이때 생성한 객체는 스프링이 관리하려면 추가적인 과정 필요)
	- spring이 요청받을때,  json->객체로 자동으로 변환할때, 즉 @RequestBody로 JSON을 받을 때는
		- 기본생성자+getter만있으면 된다
		- @RequestBody는 내부적으로 **Jackson**을 사용하는데 얘는 기본생성자 호출해 객체를 만든뒤 리플렉션을 이용해 private 필드에도 값을 **직접 주입**할 수 있어서 setter없어도 private 필드에 값넣는게 가능
	- spring이 응답보낼때 객체->json으로 자동변환해주고 응답다 작성해서 보내서 우리는 객체만 리턴하면되는데 스프링이 자동변환 안해주는 예외
		- @RestController 있고, 불변객체에 getter 있음->자동변환
		- **Getter가 없거나 필드가 private인 경우**
		- 무한 순환 참조 있음
		- 반환 **컬렉션 내부에 null 값이 있거나 타입이 모호할 때**
		- Jackson 누락
		- 반환객체가 Socket, Thread, FileInputStream 같이 **직렬화 불가능한 객체**면 에러 발생
			- 그런 필드는 @JsonIgnore로 제외하거나 DTO로 분리
	- jdbcTemplate.update()는 **INSERT, UPDATE, DELETE** 전용
		- SELECT 쿼리는 jdbcTemplate.query(), queryForObject() 등을 사용해야 함.
	- 자바의 메서드의 반환타입은 한타입만 지정이 가능하고, 그래서 한 행같이 필드를 반환할때는 하나의 객체타입으로 반환해야하는데, 객체는 개발자 설계에 따라 (id,name만, id,name,age만, 모두다)이런식으로 다르게 정의되기 때문에 spring은 자동으로 객체로 만들어줄수없다. 그래서 우리가 변환방식을 정의하는 RowMapper가 필요하다 -> 그래서  **queryForObject()는** 한 행 반환할땐 rowmapper가 필요하고 하나의 값 반환할땐 rowmapper필요없다. query()는 여러행을 반환하니 필수
---
- ## 3장 컨트롤러는 진입점의 역할만 수행해야한다 
	- ### 한 API에서 10개의 테이블을 사용해야한다면? 한 함수인 컨트롤러의 코드가 너무길어진다 ->서비스, Repositpry로 분리
	- 서비스(비즈니스 로직)
		- 컨트롤러에게 서비스를 제공한다.
		- 조건검사나 업무 처리 흐름만 담당
		- 데이터를 어떻게 쓸지를 신경씀
	- Repository(sql실행로직)
		- sql실행로직에 서비스에 들어가면 서비스 코드가 너무 복잡하고 길어짐
		- 그래서 db관련로직만 따로 reoository로 분리
	- #### 1. 좋은코드가 왜 중요한지 이해하고, 원래있던 컨트롤러코드를 보다 좋은 코드로 리팩토링한다.
		- 코드란 요구사항을 표현하는 언어이다.
		- 개발자는 요구사항을 구현하기 위해 코드를 읽고 작성한다.
		- 특히 작성된 코드를 읽는 시간이 더 많다.
			- 다음 버전출시가 바빠 코드를 마구 작성했는데, 기능을 추가할수록 코드는 엉망이 되었고 감당이 안됐다.
		- ##### 1) 함수는 최대한 작게 만들고 한가지 일만 수행하는것이 좋다
			- 유지보수가 힘들다.
				- 컨트롤러의 함수 api를 작성했는데 코드가 3000줄이 넘으면?
				- 그 함수를 읽고, 이해하는것이 너무 어렵다.
				- 그 함수를 동시에 여러명이 수정할수없다
				- 함수의 어느부분을 수정하더라도 함수전체에 영향을 미칠수있기때문에 함부로 건들수없다
				- 너무 큰기능이기 때문에 테스트도 힘들다
			- 우리가 작성한 컨트롤러 함수가 기능이 분리된다면 분리하기
				- 1. api 진입지점으로서 HTTP Body를 객체로 변환한다.
					- contoller의 역할
				- 2. 현재 유저가 있는지 없는지 확인하고 예외처리를 한다.
					- Seervice의 역할
			...
			- 분리해 코드를 작성.
				...
				public class UserService {  
					...
					public List isUserNotExist(long id) {  
				...
				public class UserController {  
				    ...
				    @PutMapping("/user")  
				    public void UpdateUser(@RequestBody UserUpdateRequest request) {  
				        userService.UpdateUser(request.getName(),request.getId());  
				    }  
				}
					```
		- ##### 2) 클래스는 작아야하며, 하나의 책임만을 가져야 한다
	- #### 2. 스프링컨테이너와 스프링빈이 무엇인지 이해한다
		1. UserController는 JdbcTemplate이 없으면 작동할 수 없다.
			-  즉, UserController는 JdbcTemplate에 의존하고 있는 상태야.
		2. 그런데 우리는 JdbcTemplate 객체를 생성하지도 않았고, 설정하지도 않았어.
			- new JdbcTemplate() 호출한 적도 없고
			- .setJdbcTemplate(...) 같은 설정도 한 적이 없지.
		3. 그런데도 UserController 안에 JdbcTemplate이 자동으로 들어가 있음.
		- 왜냐면 @RestController가 아래에 있는 클래스를 api 진입지점으로 만들어주고,스프링 빈으로 등록시키기 때문이다.
		- 스프링 빈은 거대한 컨테이너에 들어간 인스턴스다
			- 서버가 시작되면, 스프링서버 내부에 거대한 컨테이너를 만들게 된다
			- 스프링이 클래스들을 스캔해 어노테이션들이 붙은 클래스들을 자동으로 Bean으로 등록한다.
			- 빈으로 등록할때 인스턴스를 생성하고 인스턴스를 거대한 컨테이너에 담아준다
			- 이때 인스턴스 자체만 보관하는게 아닌, 다양한 정보도 추가적으로 함께 담아준다
				- 의존성 정보
					- 이 객체가 생성될때 어떤 다른빈들을 필요로 하는지, 
					- 생성자에 어떤파라미터가 필요한지(자동주입)
				- 빈의 이름과 타입
				- 라이프사이클 등
		- **개발자가 JdbcTemplate를 Bean 등록 안 해도 되는 이유와 과정**
			- Gradle로 자동으로 빈이 등록됨!
				```java 
				//build.gradle 파일 안의 코드
				implementation 'org.springframework.boot:spring-boot-starter-data-jpa'
				```
				- Gradle이 JPA 기능에 필요한 여러 라이브러리들을 다운로드하도록 해주고, Spring Boot는 서버 실행 시 spring-boot-autoconfigure에 정의된 설정에 따라 관련 객체들(예: EntityManager, JpaRepository 등)을 자동으로 Bean으로 등록해줍니다.
				- 이것때문이라는데... 사실 아래 이건 이해 못함(아래 내용 틀릴수 있음)
				- build.gradle역할: Gradle이 implementation에 적힌 라이브러리들을 자동으로 **다운로드 및 프로젝트에 추가**합니다.
				- spring-boot-starter-data-jpa : Spring Boot에서 JPA 기능을 쉽게 사용할 수 있도록 관련 라이브러리들을 묶어놓은 **스타터(Starter)**
					- spring-boot-starter 디렉토리안에 있는게 아니라 별개다
				- hibernate-core: JPA 기능을 실제로 구현한 핵심 라이브러리
				- spring-boot-autoconfigure
					- **JPA 관련 객체들을 자동으로 Bean으로 등록해주는 코드들이 들어 있는 라이브러리**
				- spring-boot-starter
					-  Spring Boot에서 특정 기능을 사용하기 위해 **기본적으로 필요한 의존성들을 묶어 놓은 빈 껍데기 라이브러리**
				- 등이 있다
		- 스프링빈으로 등록해야 스프링이 의존성있는 클래스를 넣어줄수있다 
	- #### 3. new대신 스프링컨테이너가 왜 필요한지 좋은 코드와 어떻게 연관이 있는지 이해한다.
		
		🎯 목표
		기존에는 Book 데이터를 메모리에 저장하고 있었지만,
		이제는 MySQL DB에 저장하도록 구조를 변경하고 싶음.
		단, 서비스·컨트롤러 코드 수정 없이 구현체만 교체하자!
		
		⸻
		① \[초기 단계] 직접 new 로 구현체 생성
		BookMemoryRepository repo = new BookMemoryRepository();
		•	✅ 장점: 간단하게 구현체를 직접 써서 사용
		•	❌ 단점: 코드에 구현체가 고정됨
		•	→ 나중에 DB 저장 방식으로 바꾸려면
		→ new BookMySQLRepository() 로
		→ 수백 줄의 코드 모두 수정해야 함 😱
		...
		•	✅ 장점: 추상화 → BookRepository 타입만 사용하면 구현체는 나중에 갈아끼울 수 있음
		•	❌ 단점: 여전히 직접 new 함
		•	new BookMemoryRepository() → 직접 수정해야 함
		•	인터페이스만으로는 **제어권이 여전히 개발자에게 있음**
	
		⸻
		
		③ \[개선2] 스프링 빈 + 의존성 주입(DI) 적용->제어의 역전(IoC)의 실현
		...
		•	✅ 장점:
		•	@Repository 를 붙이면 Spring이 자동으로 빈으로 등록
		•	@Service 클래스 생성자에서 **자동으로 필요한 구현체를 주입받음**
		•	구현체 교체는 이제 코드가 아닌, **빈 등록 설정**만 바꾸면 됨
		BookRepository의 구현체가 여러 개 있을 경우,
		어떤 걸 주입해야 할지 스프링이 모호함을 느낄 수 있음
			• ✅ @Primary 를 사용하면 → 우선순위를 지정할 수 있음
		
		@Repository
		@Primary
		public class BookMySQLRepository implements BookRepository { … }
		
		->즉  **“처음부터 인터페이스 + 생성자 주입 기반 구조로 짜면, 나중에 구현체만 갈아끼우면 된다”**
		제어의 역전(IoC)이란?
		
		원래 개발자가 직접 new를 호출해서 객체를 생성하던 제어권(Control) 을,
		스프링 프레임워크에게 넘김으로써
		필요한 객체를 자동으로 주입받는 구조로 바뀐 것입니다.
	- #### 4. 스프링빈을 다루는 여러방법을 이해한다
		- 1. 스프링 빈을 등록하는 방법 : **@Configuration + @Bean 기반 등록 (수동 등록 방식)**
			- @Configuration
				- 클래스에 붙이는 어노테이션
				- @Bean을 사용할 때 함께 사용해 주어야 한다.
			- @Bean 
				- 메소드에 붙이는 어노테이션
				- 메소드에서 **반환되는 객체를** 스프링빈에 등록한다.
			- 이 두가지를 같이 사용해 외부 라이브러리나 우리가 직접 어노테이션을 붙일 수 없는 클래스들을 **우회적으로 빈으로 등록**할 때 사용 (ex) jdbcTemplate)
		- 2. 스프링 빈을 등록하는 방법: **@Component 기반 등록 (자동 등록 방식)**
			- 주어진 클래스를 **스프링의 컴포넌트**로 간주한다.
			- 이 클래스들은 스프링 서버가 뜰 떄 자동으로 감지되고 스프링 빈으로 등록된다.
			- @Service, @Repository, @RestController 등은 내부에 @Component를 포함하고 있어 이 어노테이션들이 붙은 클래스도 스프링이 자동으로 감지해서 **빈으로 등록**된다.
		- 스프링빈을 주입받는 방법: 
			- 생성자(가장 권장)
				- 원래 생성자 위에 @AutoWired라는 어노테이션을 붙여줬어야 했다
				- 생성자에 있는 매걔변수에 스프링 빈을 넣어달라는 의미
				- 자동으로 등록됨
			- Setter만들고 그 위에 @AutoWired를 붙이기
				- 누군가 Setter를 사용하면 다른인스턴스로 교차되기 때문에 오작동할 요지가 있다.
			- 필드에 직접 @AutoWired 붙이기
				- private JdbcTemplate jdbcTemplate; 위에 @AutoWired붙여준다
				- 테스트를 어렵게 만드는 요인이다.
		- @Qualifier("이름")
			- 정확하게 어떤 빈을 사용할지 이름으로 지정
			- 주입 지점마다 다른 빈을 지정 가능
		- 빈주입 우선순위
			- 1순위: 주입받는 쪽에 @Qualifier("이름")
			- 2순위: @Qualifier 없음, 등록된 빈 중 @Primary 존재
				-  @Primary는 **기본값**으로 사용할 빈을 정할 때임
	- 알아야할것
		- 클래스 이름으로 메서드를 부르는 건 **static 메서드** 이다
			- **정적 메서드(static)** 는 객체 생성 없이 **클래스 이름으로 직접 호출**할 수 있어요.
			- **JVM은 객체가 없는 상태에서 바로 메서드를 실행**하려고 해요.
		- 아니 나는 생성자 **new UserRepository(…) 를 호출한 적이 없는데 왜 생성된 거지?** 
			- 바로 @어노테이션을 붙였기 때문!
			- **Spring Boot가 시작될 때**, 어노테이션 붙은 클래스 스캔
			- Spring이 객체를 **Bean으로 생성해서 내부 컨테이너에 등록**
			- 필요하면 스프링이 자기 컨테이너에서 찾고, 자동으로 넣어줌
		- 그럼 Spring이 만들어준 객체를 우리가 쓰려면?
			- 1. 먼저 클래스 안에 우리가 사용할 객체를 담기 위한 **필드 선언**
				- private final UserRepository userRepository;
			- 2. 생성자도 수정: 생성자의 매개변수로 Bean을 받아와서, 위에서 선언한 필드에 저장합니다.
			- 이렇게 작성하면 Spring이 자동으로 생성한 Bean을 우리 코드에 주입해줘
- ## 4장 JPA 사용하기
	- ### 1. 문자열 SQL을 직접 사용할때의 한계를 이해하고, 해결책인 JPA, Hibernate, Spring Data JPA가 무엇인지 이해한다
		- 한계:SQL를 문자열로 직접 작성하면..
			- 실수할수있고, 실수를 알아차리기 느리다
				- String sql="UPDATE user SET name=?;
				- 문자열 오타내도 서버는 정상 시작이 되고, 이 코드를 사용할때 되서야 에러가 난다. 즉 컴파일시점에 발견되지 않고, 런타임 시점에 발견된다
				- 오류는 런타임보다 컴파일시점에 발견하는게 용이하다.
			- 특정 데이터베이스에 종속적이게 된다.
				- mysql말고 다양한 데이터베이스가 있는데, 다른 DB로 갈아탈시에 문자열 코드 다 고쳐줘야한다.
			- 반복 작업이 많아지고 테이블을 하나만들때마다 CRUD쿼리가 항상 필요하다.
			- DB의 테이블과 객체는 패러다임이 다르다. 현재는 객체지향적 개발이기 때문에 패러다임이 다른 테이블의 직접적인 사용은 아쉽다.
				- 객체는 
					- 단순한 데이터 저장소가 아니라, 책임과 행위를 함께 지닌 존재
					- 관계 또한 테이블의 외래키가 아닌, 객체 간 참조 관계로 표현
				- 테이블은 
					- 단순히 속성의 나열이며, 행위(메서드) 에 대한 표현이 없습니다.
					- 테이블 간 관계는 코드 내부가 아닌, SQL의 JOIN 연산으로 외부적으로 구성
					- 객체의 상속을 표현하기가 힘들다.
		- JPA(Java Persistence영속성 API) :규칙이다. 규칙이 라는건 규칙일뿐이고 누군가는 이거를 코드로 구현해야한다 즉 JPA는 인터페이스라고 생각하자.
			- 객체와 RDB의 테이블을 짝지어 데이터를 영구적으로 DB에 보관하기위해 Java 진영에서 정해진 규칙
			- Java진영의 ORM이라고도 한다
				- ORM(Object-Relational Mapping)
					- Object: 객체
					- Relational: 관계형DB의 Table를 의미
					- Mapping: 객체의 정보와 테이블의 정보 둘을 짝짓는다.
			- 영속성: 서버가 재시작되어도 데이터는 영구적으로 저장된다. 
				- 서버를 종료하면 Ram에 있는 정보는 사라진다
				- 우리가 데이터베이스를 사용한것은 유저데이터에 영속성을 우리가 부여한것이다
		- Hibernate: JPA구현한 코드
			- 내부적으로 JDBC를 사용한다.
	- ### 2. Spring Data JPA를 이용해 데이터를 조회, 수정, 삭제 할 수 있다.
		- Spring Data JPA
			- Spring에서 JPA와 Hibernate를 더 쉽게 쓸 수 있게 만든 도구
			- @Repository 인터페이스만 만들어도 자동으로 SQL 실행됨
			- 메서드 이름만으로 쿼리 실행 가능
		- 1. 테이블과 유저객체를 Spring Data JPA를 이용해 매핑하자.
			- 1-1. 클래스 매핑: 클래스 위에 @Entity를 붙인다
				- 클래스 이름이 기본적으로 테이블 이름이 됨 (@Table로 수정 가능)
				- 클래스는 **DB의 테이블 1개에 대응**
				- @Entity: 이 클래스가 DB 테이블과 매핑된다는 것을 명시해 JPA가 이 객체를 관리할 수 있도록 설정
					- 엔티티: DB에서 저장되고 관리되어야할 데이터를 의미한다
			- 1-2. PK(기본키) 매핑:-> 테이블의 기본 키를 Java 객체 필드와 연결
				- id임을 알려주는 java.x.persistence경로의 @id를 붙여준다
				- 테이블의 id에는 autoincrement 속성은  @GeneratedValue를 붙여준다
					```java
					@Id
					@GeneratedValue(strategy = GenerationType.IDENTITY)//db따라 코드가 달라짐
					private Long id;		
					```
					-  **JPA의 @GeneratedValue 전략은 사용하는 DB 종류에 따라 방식이 다르며,**  **MySQL은 IDENTITY, Oracle은 SEQUENCE, 모든 DB에서 쓸 수 있는 건 TABLE이다.**
			- 1-3. 엔티티객체(JPA객체)에는 매걔변수가 하나도 없는 기본생성자가 있어야 하니 꼭 만들어주기 
				- protected 타입으로 기본생성자 만들어도 ㄱㅊ
			- 1-4. 나머지 필드에 @Column붙여주기→ 테이블의 컬럼으로 매핑 
				- 필드명과 컬럼명이 다르면 @Column(name = "...")
					- null들어갈수있는지(nullable) 여부, 길이제한(length) 를 주로 사용
					- 필드명과 컬럼명이 다를때 이름(name)지정
				- 사실 @Column은 생략할 수 있다.
					- nullable = true
					- length 문자열의경우 255
					- VARCHAR(255) 이런식으로 자동 설정됨
			- 1-5. application.yml 수정하기
				```yml
					spring:
					  jpa:
					    hibernate:
				...
				```
				-  Spring Boot에서 spring-boot-starter-data-jpa를 쓰면, 내부적으로 JPA를 실제로 구현한 구현체인 Hibernate가 작동 그래서 설정 파일에 hibernate.xxx 설정을 넣는 것
				- ddl-auto는 Hibernate가 DB 테이블을 어떻게 생성/갱신할지를 설정하는 옵션
					- none 아무 것도 하지 않음 (수동으로 테이블 준비)
					- create 실행 시 테이블 새로 생성 (기존 테이블 삭제)
					- create-drop 실행 시 생성 → 종료 시 삭제
					- update 테이블이 없으면 추가함 (기존 데이터 유지)
					- validate 테이블 구조 검증만 하고 생성은 안 함
				- show_sql = true
					Hibernate가 실행하는 SQL문을 콘솔에 출력하게 합니다.
				- format_sql = true
					콘솔에 출력되는 SQL을 가독성 있게 정렬해서 보여줍니다. (줄바꿈, 들여쓰기 등)
				- dialect = org.hibernate.dialect.MySQL8Dialect
					- dialect: 방언, DB따라 다른 문법 
					어떤 DB를 사용하는지 Hibernate에 알려주는 설정입니다.
					MySQL8Dialect: MySQL 8 버전용 SQL 문법을 사용할 수 있도록 알려줌.
					Oracle12cDialect:  Oracle DB라면이걸 쓰기
		- 2. Sql를 작성하지 않고 Spring Data JPA를 이용해 자동으로 쿼리 날리기
			- 2_1 : User 도메인 객체와 같은 위치에 UserRepository라는 인터페이스를 만들어 주자
			- 2_2 : 그 인터페이스가 JpaRepository<Entity, ID>라는 인터페이스를 상속하게해 CRUD 기능을 자동으로 사용한다. 
				- extends JpaRepository: Spring Data JPA가 제공하는 CRUD 자동 처리용 인터페이스
				- Entity: DB와 매핑된 Entity 클래스 
					- ex) User
				- ID: 해당 엔티티의 ID 필드 타입 (@Id) 
					- ex)Long
			- 이때 JpaRepository<Entity, ID>인터페이스를 상속했다면 @Repository를 생략해도 된다(이미 상속만으로 빈으로 인식함)
			- spring Data JPA 주요메서드
				- 우리가 만든 인터페이스이름.**save(new 객체)**
				- List\<객체> users= 우리가 만든 인터페이스이름.**findAll()**
					- 모든 엔티티 객체들 리스트로 반환됨
					- 엔티티를 DTO로 변환해 내보내야함
						- userRepository.findAll() 모든 User 엔티티 객체들을 가져옴
						- .stream() 리스트를 스트림으로 바꿈 (반복 작업 처리 가능)
						- .map(...) 각각의 User 엔티티 → UserResponse 객체로 변환
						- .collect(Collectors.toList()) 최종 결과를 리스트로 모음 (List\<UserResponse>로 반환)
					- 하나하나씩 열어서 매핑하는 jdbc탬플릿 이용대신 User객체를 가져와서 객체간의 변환을 하는거다 (즉 매핑이 자동화)
						- **DBC Template 방식** .query
							- ResultSet → UserResponse 객체를 직접 매핑해야 했음 즉, DB에서 값을 꺼내면서 일일이 가공해서 집어넣는 작업 필요
						- JPA 방식 
							- 먼저 DB에서 User 엔티티 객체들을 가져오고
							- 그걸 stream으로 DTO(UserResponse) 로 변환만 해주면 끝
						- 즉 컬럼 추가/삭제 시 **모든 SQL과 RowMapper 수정** 필요했는데, JPA 방식은 결과를 받아오는 필드를 따로 작성하지 않아서(매핑), **엔티티 클래스만 수정하면 자동으로 쿼리와 매핑이 반영**되어 **유지보수가 훨씬 간단하고 안정적**입니다. 
				- Optional\<객체> user =우리가 만든 인터페이스이름.**findById(id)**
					- Optional로 반환되므로 null 처리 걱정 줄어듦
				-  인터페이스이름.**deleteName(name)** 하면 지워짐
			- spring Data JPA 사용자 정의 메서드
				- 이름을 기준으로 조회하는건 없어서 우리가 인터페이스에 들어가 findByName를 선언해준다
					- findByName- **메서드 이름 규칙만 맞추면 자동으로 SQL 생성됨**
				- 형식 구절1 by 구절2
					- 암기할필요x
				- by앞
					- find: 1건의 객체, Optional<타입>을 반봔
					- findAll : List<타입> 반환 (n건의 결과물)
					- exists  : 존재여부를 boolean으로 반환
					- count: sql의 결과개수를 쓴다 반환타입은 long이다
				- by뒤
					- 각구절은 and나 or로 조합할수있다.
					- GreaterThan: 초과
					- GreaterThanEqual: 이상
					- LessThan: 미만
					- LessthanEqual: 이하
					...
					- StartsWith: ~로 시작하는
					- EndsWith: ~로 끝나는
			- jpa를 활용하면 업데이트 sql를 직접 날리는게 아니라 유저의 객체를 가져와서 업데이트하게됨
				- 이때 User에 getName의 역할을하는 updateName이라는 메소드를 만들어서 가져온객체의 이름을 변경하고
				- 인터페이스이름.save(가져온객체) 
					- 이는 변경 감지 (Dirty Checking)라고한다
					- > @Transactional 안에서 기존 엔티티를 조회해 수정만 했다면,  save() 없이도 트랜잭션이 끝나는 시점에 변경된 내용을 자동으로 감지해서 UPDATE SQL을 날림. 다만, 신규 저장이거나 트랜잭션이 없다면 save()가 필요하다.
			- 스프링은 JpaRepository<Entity, ID>를 구현받는 Repository에 대해 자동으로 SimpleJpaRepository 기능을 사용할 수 있게도 해준다.
			- SimpleJpaRepository 코드를 열어보면, 조금 복잡한 코드들을 확인할 수 있는데, 이게 바로JPA 코드이다. Spring Data JPA 를 사용하는 덕분에 복잡한 JPA 코드를 직접 사용하는 게 아니라 기능로써 사용할 수 있게 되는 것이다.
			- Spring Data JPA를 직접사용하지만 사실은 Spring Data JPA 안의 JPA 구현체인 Hibernate의 JDBC를 쓰고 있다 
		- 새로알게 된것 자바에서 자주 보이는 Optional\<T>
			-  null 때문에 생기는 **에러나 혼란을 줄이기 위한 안전한 “포장지”** 같은 클래스
			- 1. 옵셔널 객체만들기
				- Optional\<객체 타입> name = Optional.of("승현");
			- **2. 값이 있는지 확인하고 꺼내기**
				- isPresent() 값이 있으면 true, 없으면 false
				- ifPresent(람다) 값이 있으면 실행
				- orElse(기본값) 값이 없을 때 기본값 반환
				- orElseThrow() 값이 없으면 예외 발생
				- get() 안에 있는 값 꺼내기 (값이 없으면 예외 발생) ← 주의! 가급적 쓰지 말기  (옵셔널의 의미가 없는 메서드)
			-  **findById(id)**하면 이런 옵셔널 타입으로 반환되니 위의 메서드가 쓰인것.
