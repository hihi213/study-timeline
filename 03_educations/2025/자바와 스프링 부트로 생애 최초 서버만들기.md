...
- ## 4장 JPA 사용하기
	...
	- ### 2. Spring Data JPA를 이용해 데이터를 조회, 수정, 삭제 할 수 있다.
		- 테이블과 유저객체를 JPA를 이용해 매핑하자.
			- 1. 클래스 매핑: 클래스 위에 @Entity를 붙인다
			...
			- 2. PK(기본키) 매핑:-> 테이블의 기본 키를 Java 객체 필드와 연결
			...
			- 3. 엔티티객체(JPA객체)에는 매걔변수가 하나도 없는 기본생성자가 있어야 하니 꼭 만들어주기 
				- protected 타입으로 기본생성자 만들어도 ㄱㅊ
			- 4. 나머지 필드에 @Column붙여주기→ 테이블의 컬럼으로 매핑 
			...
			- 5. application.yml 수정하기
				...
				- ddl-auto는 Hibernate가 DB 테이블을 어떻게 생성/갱신할지를 설정하는 옵션
					- none(수동으로 테이블 준비됨)/create(기존테이블 삭제후 새로 생성)/ 등등
					- DDL crud 기억나지?
				- show_sql = true
