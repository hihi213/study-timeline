		...
		1.  HTTP  **요청** 
			...
			- 필수: 첫번째 줄(요청라인) 
				...
				- 요청 URL: 누구에게 요청을 보내야 하는지
					...
					```Java
					@RestController
						@RequestMapping("/컨트롤러클래스식별자") // 공통 prefix
	...
	- #### 3. 스프링 서버를 이용해 Database에 접근하고 데이터를 저장, 조회, 업데이트, 삭제할 수 있다.
		...
		- ##### 3. 유저 컨트롤러에 들어가서 메모리에 저장하던걸 mysql로 저장하게 바꿔주기
			...
			- ###### 3_5) SQL를 만들어 문자열변수로 만들고 jdbcTemplate로 조작한다
				...
				- RowMapper는 DB 테이블의 한 줄(Row) 을 Java 객체로 바꾸는 도구이고,
					- maprow()를 오버라이딩하기위한 인터페이스
					- ResultSet: DB 결과 전체를 담는 자바 객체
					- 이때 ctrl+o를 눌러 maprow() 오버라이딩을 해줘야해
					- maprow()
						- ResultSet의 한줄을 Java 객체(T)로 변환하는 메서드
				- JdbcTemplate.query()는 그걸 반복해서 리스트로 만들어주는 메서드입니다.
					- query() 메서드를 사용해 SQL을 실행하고 결과를 받아옴
					- 두 번째 인자는 결과를 **어떤 객체로 변환할지**를 지정하는 RowMapper로 설정한다
					
				- 1.JdbcTemplate이 SQL을 DB에 전달하고	
				- 2.결과를 ResultSet 형태로 받아옴
				- 3.RowMapper\<UserResponse> 가 ResultSet의 각 행(row)을 UserResponse 객체로 바꾸고
				- 4. 변환된 객체들을 리스트에 담아서
				- 5. List\<UserResponse> 형태로 리턴해줍니다
