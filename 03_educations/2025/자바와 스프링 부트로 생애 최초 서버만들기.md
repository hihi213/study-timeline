...
- ## 3장 컨트롤러는 진입점의 역할만 수행해야한다 
	...
	- #### 2. 스프링컨테이너와 스프링빈이 무엇인지 이해한다
		...
		- build.gradle의 필요성
			...
			```
			- 의존성을 적은 starter 
			- spring-boot-autoconfigure로 bean을 등록해주는 파일
		- Jdbc 탬플릿도 스프링 빈이다. ->그래서 자동으로 스프링이 주입한다.
		- build.gradle를 통해 의존성을 설정했고, jdbc가 스프링빈으로 등록하는 코드
		- build.gradle를 통해 의존성을 설정했고, jdbc가 스프링빈으로 등록하는 코드 이건또 뭔소리일까
